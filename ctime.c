...
typedef struct day_range
{
  unsigned int FirstDayIndex;
  unsigned int LastDayIndex;
} day_range;
...
static day_range DayRange(timing_file_date A, timing_file_date B)
{
  double DayInTime = 86400.0;
  double TimeDiff = difftime(B.E, A.E);
  day_range Result;
  Result.FirstDayIndex = 0;
  Result.LastDayIndex = (unsigned int) ceil(TimeDiff / DayInTime);
  return Result;
}

static unsigned int DaySpan(day_range X)
{
  return X.LastDayIndex - X.FirstDayIndex;
}
...
CSV(timing_entry_array Array, char *TimingFileName)
{
    ...
    fprintf(stderr, "ordinal, date, duration, status\n");
    {for(EntryIndex = 0;
         EntryIndex < Array.EntryCount;
         ++EntryIndex, ++Entry)
    {
        fprintf(stderr, "%d, ", EntryIndex);
        PrintDate(Entry->StartDate);
        if(Entry->Flags & TFEF_Complete)
        {
            fprintf(stderr, ", %0.3fs, %s", (double)Entry->MillisecondsElapsed / 1000.0,
                    (Entry->Flags & TFEF_NoErrors) ? "succeeded" : "failed");
        }
        else
        {
            fprintf(stderr, ", (never completed), failed");
        }
...
PrintTime(double Milliseconds)
{
        ...
        ++PartIndex)
    {
        ...
        if(This > 0)
        {
            fprintf(stderr, "%d %s%s, ", (int)This, Parts[PartIndex].Name,
                (This != 1) ? "s" : "");
        }
        Q -= This*MsPer;
    }

    fprintf(stderr, "%0.3f seconds", (double)Q / 1000.0);
}
...
//                double AverageMs = Group->TotalMs / (double)Group->Count;
    ...
    fputc('+', stderr);
    for(BucketIndex = 0; BucketIndex < GRAPH_WIDTH; ++BucketIndex) {fputc('-', stderr);}
    fputc(' ', stderr);
    PrintTime(0);
    fputc('\n', stderr);
    fputc('\n', stderr);
        ...
        --LineIndex)
    {
        ...
        if(LineIndex == (GRAPH_HEIGHT - 1))
        {
            fprintf(stderr, " %u", MaxCountInBucket);
        }
        fputc('\n', stderr);
    }
    fputc('+', stderr);
...
Stats(timing_entry_array Array, char *TimingFileName)
{
    ...
    timing_file_date OriginDate = {0};
    double AllMs = 0;

    day_range TotalDays = {0};
    double TotalDaySpan = 0;
    ...
    if(Array.EntryCount >= 2)
    {
        ...
        TotalDays = DayRange(Array.Entries[0].StartDate, Array.Entries[Array.EntryCount - 1].StartDate);
        OriginDate = Array.Entries[0].StartDate;
        TotalDaySpan = DaySpan(TotalDays);
    }
    else
    {
      fprintf(stderr, "%s: More than two entries are needed to show stats\n", TimingFileName);
      return;
    }
        ...
        ++EntryIndex, ++Entry)
    {
        if(Entry->Flags & TFEF_Complete)
        {
            ...
            day_range ThisDay = DayRange(OriginDate, Entry->StartDate);
            if(LastDayIndex != ThisDay.LastDayIndex)
            {
                LastDayIndex = ThisDay.LastDayIndex;
                ++DaysWithTimingCount;
            }
            ...
            {
              int GraphIndex = (int)(((double)DaySpan(ThisDay)/TotalDaySpan)*(double)GRAPH_WIDTH);
                UpdateStatGroup(TotalGraph.Buckets + GraphIndex, Entry);
            }

            {
                int GraphIndex = (int)(ThisDay.LastDayIndex - (TotalDays.LastDayIndex - GRAPH_WIDTH));
                if(GraphIndex >= 0)
                {
    ...
    fprintf(stderr, "\n%s Statistics\n\n", TimingFileName);
    fprintf(stderr, "Total complete timings: %d\n", WithErrors.Count + NoErrors.Count);
    fprintf(stderr, "Total incomplete timings: %d\n", IncompleteCount);
    fprintf(stderr, "Days with timings: %d\n", DaysWithTimingCount);
    fprintf(stderr, "Days between first and last timing: %d\n", DaySpan(TotalDays));
    PrintStatGroup("Timings marked successful", &NoErrors);
    PrintStatGroup("Timings marked failed", &WithErrors);

    PrintGraph("All", TotalDaySpan, &TotalGraph);
